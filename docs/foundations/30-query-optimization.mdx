  ...
  steps={[
    ...
    },
    {
      ...
      table: {
        ...
        rows: [["Seq Scan on orders, Filter: date(created_at) = ...", "40,000 (all)"]],
      },
      rows: { drop: [0] },
    },
    {
      ...
      table: {
        columns: ["plan", "rows read"],
        rows: [["Index Scan using idx_created, range [00:00, next day)", "~240"]],
      },
      rows: { match: [0] },
    },
    {
      title: "4 · same rows, ~160x less work — result",
      table: {
        name: "result",
        result: true,
        columns: ["approach", "index used?", "rows scanned"],
        rows: [["date() wrapper", "no", "40,000"], ["half-open range", "yes", "~240"]],
      },
    },
  ]}
/>
...
:::danger[💥 Break]
Write it as `WHERE created_at::date = $1`. On tens of thousands of rows the index on
